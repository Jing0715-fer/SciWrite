...
Output JSON only.`;
    ...
    // SAFETY GATE (F2 fix): only accept suggestions whose marker is ACTUALLY in
    // the missing set. The LLM can hallucinate markers like "[3]" that are
    // perfectly valid citations — replacing those would destroy correct
    // citations. Whitelist against the authoritative missing list.
    const missingMarkerSet = new Set(missing.map((m) => m.inner));
    const suggestions = (Array.isArray(parsed.suggestions) ? parsed.suggestions : [])
      .filter((s: any) => s.database && s.query)
      .filter((s: any) => {
        const markerNum = String(s.marker || "").replace(/[\[\]]/g, "").trim();
        return markerNum && missingMarkerSet.has(markerNum);
      });
    ...
    for (const suggestion of suggestions.slice(0, 5)) {
      try {
        ...
        if (dbResult.items.length > 0) {
          // SAFETY GATE (F1 fix — "hallucination factory"): the search query's
          // first result is NOT necessarily the right reference. Score each
          // candidate's title+abstract against the paragraph context and only
          // accept one that is actually topically related. If nothing passes,
          // leave the marker for the [$REF] fallback — inserting an unrelated
          // paper as "the correct citation" is worse than leaving a gap.
          const candidate = dbResult.items
            .slice(0, 5)
            .map((item: any) => ({
              item,
              score: topicalityScore(
                body.slice(0, 2000),
                `${item.title || ""} ${item.abstract || ""} ${item.journal || ""}`
              ),
            }))
            .sort((a: any, b: any) => b.score - a.score)
            .find((c: any) => c.score >= 0.03);
          if (!candidate) {
            continue; // no topically-relevant result — do not insert junk
          }
