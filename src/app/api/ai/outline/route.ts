...
export async function POST(req: NextRequest) {
  ...
  (async () => {
    try {
      send("step", { status: "started", message: "Loading project..." });
      ...
      send("step", { status: "progress", message: "Designing outline prompt..." });
      ...
      const validFormats = ["abstract", "intro", "background", "methods", "results", "discussion", "conclusion"];
      const validScenarios = ["literature-review", "protein-structure", "sequence-analysis", "mechanism", "comparative", "clinical", "custom"];
...
Output JSON only. Ensure each paragraph builds logically on the previous.`;
      ...
      send("step", { status: "progress", message: "Calling LLM to generate outline..." });
      ...
      send("step", { status: "progress", message: "Parsing outline structure..." });

      const parsed = safeParseJSON(raw, { summary: "", outline: [] });

      const outline: OutlineItem[] = (Array.isArray(parsed.outline) ? parsed.outline : [])
        .filter((o: any) => validFormats.includes(o.format) && validScenarios.includes(o.scenario))
        .map((o: any) => ({
          ...
          suggestedQueries: Array.isArray(o.suggestedQueries)
            ? o.suggestedQueries
                ...
                .slice(0, 3)
                .map((q: any) => ({ database: String(q.database).toLowerCase(), query: String(q.query) }))
            : [],
        }));

      send("step", { status: "done", message: `Outline generated with ${outline.length} paragraphs.` });
      send("complete", {
