        ...
        for (let i = 0; i < itemsToSave.length; i++) {
          ...
          try {
            ...
            if (isPubMed || isRcsbWithPub || isWebWithUrl) {
              try {
                // FIX: RCSB entries with a linked publication use the PMID
                // (extra.pmid) as externalId — not the PDB ID — so reference
                // identity (type:pubmed + externalId) matches the same paper
                // gathered directly from PubMed (dedup + audits rely on it).
                const ref = await db.reference.create({
                  data: {
                    type: isPubMed ? "pubmed" : isRcsbWithPub ? "pubmed" : "web",
                    externalId: isRcsbWithPub
...
${sectionStructureContext ? "When a PROTEIN STRUCTURE ANALYSIS block is provided, USE the specific computed numeric values (resolution, % helix/sheet, ligand chain:resSeq, Ramachandran % favoured, B-factor mean, SASA % exposed, H-bond count, pI, BSA) — they are REAL values from the actual PDB file. Quote them precisely and connect them to biological function. NEVER fabricate structural metrics." : ""}`;
        ...
        for (let i = 0; i < renumberedContents.length; i++) {
          ...
          // or end of paragraph).
          // ACCURACY FIX: the density-injection sentence ("Further reading on
          // this topic: [n] Author (year) — title...") has the SAME defect —
          // v101-1's regex only matched "Further context", letting "Further
          // reading" blocks survive into the final article with locally-numbered
          // citations that point at the WRONG references after global
          // renumbering. Strip both patterns.
          cleaned = cleaned.replace(
            /\s*(Further context on this topic is provided by|Further reading on this topic)[^\n]*(?:\.[^\n]*)*/gi,
            ""
          );
