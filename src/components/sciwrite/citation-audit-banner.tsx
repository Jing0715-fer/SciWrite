...
export function CitationAuditBanner({ articleId }: { articleId: string }) {
  ...
  // Adversarial review (hostile-critic claim-level verification + surgical
  // removal of unsupported citations) — separate from the deterministic audit.
  const [advRunning, setAdvRunning] = React.useState(false);
  const [advReport, setAdvReport] = React.useState<any>(null);
  ...
  const runAdversarial = React.useCallback(async () => {
    setAdvRunning(true);
    setError(null);
    try {
      const res = await fetch(`/api/articles/${articleId}/adversarial-review`, {
        method: "POST",
      ...
      if (!res.ok) {
        const txt = await res.text().catch(() => "");
        throw new Error(txt || `Adversarial review failed (${res.status})`);
      }
      const data = await res.json();
      setAdvReport(data.report);
      setOpen(true);
      // The article content may have changed (unsupported citations removed)
      // → re-run the deterministic audit + reload so the viewer refetches.
      await runAudit(false);
      window.location.reload();
    } catch (err: any) {
      setError(err?.message || "Adversarial review failed.");
    } finally {
      setAdvRunning(false);
    }
  }, [articleId, runAudit]);
      ...
      <div className="flex items-center gap-2 px-3 py-2">
        ...
        <div className="ml-auto flex items-center gap-1">
          ...
          >
            ...
            {advRunning ? "Reviewing…" : "Adversarial review"}
          </Button>
          <Button
            variant="ghost"
            size="sm"
            className="h-6 px-2 text-[10px] gap-1"
            disabled={state === "deep-loading" || advRunning}
            ...
            <div className="mt-2 rounded border border-emerald-300/50 dark:border-emerald-700/50 bg-emerald-50/40 dark:bg-emerald-950/15 p-2.5 space-y-1.5">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[10px] text-muted-foreground ml-auto">
                  {advReport.supported} supported · {advReport.partial} partial ·{" "}
                  {advReport.unsupported} unsupported
                  {advReport.removed > 0 ? ` · ${advReport.removed} removed` : ""}
                </span>
              </div>
              {(advReport.removedCitations || []).length > 0 && (
                <div className="space-y-1">
                  <p className="text-[10px] font-medium text-foreground/80">
                    Removed citations (verified unrelated to the claims they were attached to):
                  </p>
                  {(advReport.removedCitations || []).slice(0, 10).map((rc: any, i: number) => (
                    <div key={`rc-${i}`} className="flex gap-1.5 text-[10px] leading-snug">
                      <span className="font-mono font-semibold shrink-0">[{rc.n}]</span>
                      <span className="text-muted-foreground">
                        {rc.reason} ({rc.confidence}%) — “{rc.sentence?.slice(0, 90)}…”
                      </span>
                    </div>
                  ))}
                </div>
              )}
              {(advReport.flagged || []).length > 0 && (
                <details className="text-[10px]">
                  <summary className="cursor-pointer text-muted-foreground">
                    {advReport.flagged.length} flagged for manual verification
                  </summary>
                  <div className="mt-1 space-y-1 max-h-40 overflow-y-auto">
                    {(advReport.flagged || []).slice(0, 20).map((fc: any, i: number) => (
                      <div key={`fc-${i}`} className="flex gap-1.5 leading-snug">
                        <span className="font-mono font-semibold shrink-0">[{fc.n}]</span>
                        <span className="text-muted-foreground">
                          {fc.verdict} ({fc.confidence}%) — {fc.reason}
                        </span>
                      </div>
                    ))}
                  </div>
