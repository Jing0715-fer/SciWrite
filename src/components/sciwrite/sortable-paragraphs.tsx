...
import { parseCitationsBlock, type CitationRef } from "./markdown-citations";
...
interface Props {
   ...
   * data so that ParagraphCard hover tooltips resolve [20] etc. correctly. */
  articleContent?: string;
}

export function SortableParagraphs({ paragraphs, projectId, articleContent }: Props) {
  const { t } = useI18n();
  ...
  const reorderMut = useMutation({
    mutationFn: async (updates: { id: string; order: number }[]) => {
      const results: Promise<{ paragraph: any }>[] = [];
...
function SortableParagraph({
  ...
  return (
    <div ref={setNodeRef} style={style} className="relative group">
      ...
      <ParagraphCard paragraph={paragraph} projectId={projectId} index={index} articleContent={articleContent} />
    </div>
  );
}
