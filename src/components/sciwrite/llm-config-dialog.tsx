...
export function LLMConfigDialog({ open, onOpenChange }: Props) {
  ...
  const detect = React.useCallback(async () => {
      setLoading(true);
      try {
        // Hit /api/llm/refresh (POST) which actually clears the in-process +
        // disk probe cache and synchronously re-probes every CLI adapter —
        // a plain GET /api/llm-config would only return the stale cached
        // result and the "redetect" button would do nothing.
        const res = await fetch("/api/llm/refresh", { method: "POST" });
        if (!res.ok) {
          const errBody = await res.json().catch(() => ({} as any));
          throw new Error(errBody?.message || `HTTP ${res.status}`);
        }
        // refresh already re-probes synchronously — fetch /api/llm-config
        // again to populate the dialog with the up-to-date shape.
        const data = await fetch("/api/llm-config").then((r) => r.json());
        setConfig(data);
        await loadSelection();
      } catch (e: any) {
