...
export function ParagraphCard({ paragraph, projectId, index, articleContent }: Props) {
  ...
  // Use global refs when available (composed article), otherwise fall back
  // to paragraph-local refs.
  const effectiveRefs = globalArticleRefs.length > 0
    ? globalArticleRefs
      ...
      <div className="px-4 py-3 relative paper-surface" ref={bodyRef} onMouseUp={handleMouseUp}>
        {editing ? (
          ...
          <MarkdownCitations
            ...
            suppressRefList={globalArticleRefs.length > 0}
          />
        )}
