...
async function callAnthropic(prompt: string, system?: string, model?: string, opts?: { temperature?: number; maxTokens?: number }): Promise<string> {
  const Anthropic = (await (eval("import"))("@anthropic-ai/sdk")).default;
  const apiKey = process.env.ANTHROPIC_API_KEY;
  if (!apiKey) throw new Error("ANTHROPIC_API_KEY not set");
  const client = new Anthropic({ apiKey });
  // FIX: temperature/maxTokens were never passed through — the hardcoded
  // max_tokens=4096 truncated long outputs (truncated "### Citations" lists
  // produced out-of-range citations) and temperature:0 audit calls were
  // non-deterministic.
  const resp = await client.messages.create({
    model: model || "claude-3-5-sonnet-latest",
    max_tokens: Math.max(1024, Math.min(opts?.maxTokens ?? 8192, 32768)),
